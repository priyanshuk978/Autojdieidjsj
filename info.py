...
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002793834729'))  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002694459681'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002805287537'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002610557165')  # Channel/Group ID for force sub (make sure bot is admin)
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002610557165').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002610557165').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-1002851363855'))  # Log channel id (make sure bot is admin)
...
MOVIE_UPDATE_CHANNEL_LNK = environ.get('MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/+Htq8Zy9FNkU1ZDY1')
OWNERID = int(os.environ.get('OWNERID', '6725874739'))  # Replace with the actual admin ID
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/MovixNest')  # Support group link (make sure bot is admin)
